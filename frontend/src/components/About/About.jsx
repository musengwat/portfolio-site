...
const AboutHeader = ({ paragraph }) => {
  return (
    <motion.div className="about__header" variants={itemVariants}>
      <h2 className="about__title">About Me</h2>
      <p className="about__subtitle">{paragraph} </p>
    </motion.div>
  );
};

const AboutImage = ({ forHire }) => {
  return (
    <div className="about__image-container">
      <img
        src={'/assets/images/thomas-3.jpg'}
...
const BioDetailCard = ({ personalInfo }) => {
  return (
...
const AboutContent = ({ title, personalInfo }) => {
  return (
    <div
      style={{
        display: 'flex',
    ...
    >
      <h3 className="about__bio-title">{title}</h3>
      <p className="about__bio-text">
        I'm a passionate full-stack developer with {personalInfo.experience} years of experience
        crafting digital experiences that drive business growth. My specialization is in building
        web & mobile products to transform complex challenges into elegant, accessible solutions
        that delight users and empower development teams. I believe in writing clean, maintainable
        code and creating applications that users love.
      </p>
      <p className="about__bio-text">
        From Walmart's pickup towers to UnitedHealth's design systems, I've delivered high-impact
        applications that transform how millions of users interact with technology. Additionally, my
        efforts have saved my employers over 50,000 development hours, over $7 million in labor
        costs and over 3 million customer hours.
      </p>
      <p className="about__bio-text">
        When I'm not coding, you'll find me exploring the gorgeous scenery of Northwest Arkansas,
        working on my 15' Chevy SS, or organizing pick-up games of Counter-Strike 2.
      </p>
      <BioDetailCard personalInfo={personalInfo} />
    </div>
  );
};

const AboutHighlights = ({ highlights }) => {
  return (
...
const QuoteSection = ({ quote, citeation }) => {
  return (
    <motion.div className="about__philosophy" variants={itemVariants}>
      <div className="about__philosophy-content">
        ...
        <cite className="about__quote-author">- {citeation}</cite>
      </div>
    </motion.div>
  );
};
  ...
  return (
    <section className="about">
      <div className="container">
        ...
        >
          <AboutHeader paragraph="Passionate developer creating digital experiences that matter" />

          <div className="about__grid">
            <motion.div className="about__personal" variants={itemVariants}>
              <AboutImage forHire={personalInfo.forHire} />
              <AboutContent title={`Hello! I'm Thomas Musengwa`} personalInfo={personalInfo} />
            </motion.div>
          ...
          <QuoteSection
            quote="If we want users to like our software, we should design it to behave like a likable
                person."
            citeation="Alan Cooper"
          />
        </motion.div>
      </div>
    </section>
